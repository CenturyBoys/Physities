...
class T:
    ...
    @classmethod
    def xxx(cls, other: int | float, operation_type: OperationType):
        operations = {
        ...
        if cls.from_default and cls.to_default:
            cls.from_default = cls.__compose_function(
                inner_func=cls.from_default,
                outer_func=lambda x: operations[operation_type](x, other),
            )
        else:
            cls.from_default = lambda x: operations[operation_type](x, other)
    ...
    def __mul__(cls, other):
        if cls.value is None:
            if isinstance(other, (int, float)):
                cls.xxx(other=other, operation_type=OperationType.MULTIPLICATION)
        return cls(
            from_default=cls.from_default,
            to_default=cls.to_default,
        )
