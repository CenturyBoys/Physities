...
class TestDimension:
    ...
    def test_instantiation_success():
        obj_dimension = Dimension(dimensions_tuple=tuple(i for i in BaseUnit))
    ...
    def test_invalid_type_instantiation():
        ...
        for invalid_tuple in invalid_tuples:
            ...
            assert str(error.value) == f"dimensions_tuple is not of the type {type(tuple)}."
    ...
    def test_invalid_length_instantiation():
        ...
        for invalid_tuple in invalid_tuples:
            with pytest.raises(ValueError) as error:
                obj_dimension = Dimension(dimensions_tuple=invalid_tuple)
            assert str(error.value) == f"Invalid length of tuple. Expected {len(BaseUnit)}, but got {len(invalid_tuple)}."

    @staticmethod
    def test_new_methods_invalid_power():
        invalid_powers = [[], "1", {}, ()]
        new_methods = [Dimension.new_time, Dimension.new_length, Dimension.new_amount, Dimension.new_mass, Dimension.new_temperature]
        for method in new_methods:
            for invalid_power in invalid_powers:
                with pytest.raises(TypeError) as error:
                    result = method(power=invalid_power)
                assert str(error.value) == "The exponentiation must be a int or a float."

    @staticmethod
    def test_new_methods_success():
        new_methods = [Dimension.new_time, Dimension.new_length, Dimension.new_amount, Dimension.new_mass,
        ...
        for index in range(len(new_methods)):
            expec_dim_tuple = [0 for i in BaseUnit]
    ...
    def test_addition_invalid():
        ...
        for test in tests:
            ...
            assert str(error.value) == "Dimension only allow addition between same instance."
    ...
    def test_subtraction_invalid():
        ...
        for test in tests:
            with pytest.raises(TypeError) as error:
                result = test[0] - test[1]
            assert str(error.value) == "Dimension only allow subtraction between same instance."
    ...
    def test_multiplication():
        ...
        assert result_3.dimensions_tuple == (.5, .5, .5, .5, -.5)
    ...
    def test_multiplication_invalid():
        ...
        dimension_2 = Dimension.new_instance(dimensions_tuple=(1, 1, 1, 1, -1))
        dimension_3 = B()
        tests = [(dimension_1, dimension_2), (dimension_1, dimension_3), (dimension_1, ()), (dimension_3, dimension_1)]
        for test in tests:
            with pytest.raises(TypeError) as error:
                result = test[0]*test[1]
            assert str(error.value) == "Dimension only allow multiplication with int or floats."
    ...
    def test_pow_invalid():
        ...
        dimension_3 = B()
        tests = [(dimension_1, dimension_2), (dimension_2, dimension_1), (dimension_1, dimension_3), (dimension_2, 1)]
        for test in tests:
            with pytest.raises(TypeError) as error:
                result = test[0] ** test[1]
